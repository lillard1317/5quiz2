    ...
    <div style={{ minHeight:"100vh", background:"#0f0f1a", fontFamily:"-apple-system,Hiragino Sans,sans-serif", color:"#eee" }}>
      ...
      <div style={{ background:"rgba(255,255,255,0.03)", borderBottom:"1px solid rgba(255,255,255,0.07)", padding:"48px 16px 12px" }}>
        <div style={{ marginBottom:10 }}>
          <button onClick={onMenu} style={{ background:"rgba(255,255,255,0.08)", border:"1px solid rgba(255,255,255,0.15)", color:"#fff", padding:"8px 14px", borderRadius:10, cursor:"pointer", fontSize:13, fontWeight:700 }}>← 戻る</button>
        </div>
      ...
      <div style={{ maxWidth:460, margin:"0 auto" }}>
        ...
        <div style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.12)", borderRadius:20, padding:20, marginBottom:16 }}>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
            {history.map((h, i) => (
              <button key={i} onClick={() => onReview(i)}
        ...
        <div style={{ display:"flex", gap:12 }}>
          <button onClick={() => onReview(0)} style={{ flex:1, padding:16, borderRadius:14, border:"1px solid rgba(255,255,255,0.2)", background:"rgba(255,255,255,0.07)", color:"#fff", fontSize:14, cursor:"pointer", fontWeight:700 }}>解説を見る</button>
...
function App() {
  ...
  if (mode === "reference") return <><ReferenceScreen onBack={() => setMode("menu")} />{tabBar}</>;
  if (mode === "settings") return <><SettingsScreen onBack={() => setMode("menu")} allQCount={allQuestions.length} />{tabBar}</>;
  if (mode === "menu") return <><MenuScreen allQuestions={allQuestions} onStart={handleStart} />{tabBar}</>;
  if (mode === "quiz") return <QuizScreen questions={questions} onFinish={handleFinish} onMenu={() => setMode("menu")} />;
  if (mode === "result") return <ResultScreen history={history} score={score} totalQ={questions.length} onRetry={() => setMode("menu")} onReview={(i) => { setReviewStartIdx(i); setMode("review"); }} />;
  if (mode === "review") return <ReviewScreen history={history} startIdx={reviewStartIdx} onBack={() => setMode("result")} />;
  return null;
}
